...
class Camera(nn.Module):
    def __init__(
        ...
        if 'ego_pose' in self.meta.keys(): # TODO 是这里使用的ego_pose
...
def loadmask(cam_info: CameraInfo, resolution, resize_mode):
    ...
    if cfg.data.type == 'Blender':
        ...
        if 'ego_mask' in cam_info.metadata:
            masks['original_ego_mask'] = PILtoTorch(cam_info.metadata['ego_mask'], resolution, resize_mode=resize_mode).clamp(0, 1).bool()
            del cam_info.metadata['ego_mask']
        
        if 'car_mask' in cam_info.metadata:
            masks['original_car_mask'] = PILtoTorch(cam_info.metadata['car_mask'], resolution, resize_mode=resize_mode).clamp(0, 1).bool()
            del cam_info.metadata['car_mask']
