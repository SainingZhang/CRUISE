...
dair_track2label = {"Car": 0, "Tricyclist": 1, "Motorcyclist": 2, "Van": 3,"Truck":4, "Cyclist": 5,"Pedestrian":6,"Bus":7,"misc": -1}
...
specified_sequence_id = cfg.specified_sequence_id

with open('/your/data/dair-v2x/V2X-Seq-SPD/cooperative/data_info.json', 'r') as file:
...
with open('/your/data/dair-v2x/V2X-Seq-SPD/infrastructure-side/data_info.json', 'r') as file:
    data_info_data = json.load(file)
...
with open('/your/data/dair-v2x/V2X-Seq-SPD/vehicle-side/data_info.json', 'r') as file:
    data_info_data = json.load(file)
...
def get_obj_pose_tracking(datadir, selected_frames, ego_poses, cameras=[0, 1]):
    ...
    for tracklet in tracklets_str:
        ...
        if object_class in ['Pedestrian', 'misc']:
            continue
...
def generate_dataparser_outputs(
    ...
    source_path = "/your/data/dair-v2x/V2X-Seq-SPD"
    ...
    lidar_to_novatel = get_extrinsics_lidar_to_novatel(vehicle_lidar_to_novatel_json)
    ...
    if not os.path.exists(os.path.join(colmap_basedir, 'triangulated/sparse/model')):
        from script.dair.colmap_dair_full import run_colmap_dair
        run_colmap_dair(result)
