...
class StreetGaussianModel(nn.Module):
    ...
    def setup_functions(self):
        ...
        # Build object model but disable training
        if self.include_obj:
            for track_id, obj_meta in self.obj_info.items():
                model_name = f'obj_{track_id:03d}'
                model = GaussianModelActor(model_name=model_name, obj_meta=obj_meta)
                # 禁用所有参数的梯度
    ...
    def parse_camera(self, camera: Camera):
        ...
        # object (build scene graph) - 只在render_actors为True时添加actor到渲染列表
        self.graph_obj_list = []
        if self.include_obj and self.render_actors:  # 修改这里的条件
    ...
    def get_rotation(self):
        ...
        for i, obj_name in enumerate(self.graph_obj_list):
            ...
            rotations_global = quaternion_raw_multiply(obj_rot[None], rotations_local) # [N, 4]
    ...
    def get_xyz(self):
        ...
        if self.get_visibility('background'):
            ...
            if self.use_pose_correction:
                xyzs_bkgd = self.pose_correction.correct_gaussian_xyz(self.viewpoint_camera, xyzs_bkgd)
    ...
    def training_setup(self, exclude_list=[]):
        ...
        for model_name in self.model_name_id.keys():
            ...
            model: GaussianModel = getattr(self, model_name)
            # 只为非actor模型设置训练参数
            # if not model_name.startswith('obj_'):
    ...
    def get_xyz_edit(self):
        ...
        for i, obj_name in enumerate(self.graph_obj_list):
            ...
            if obj_name in swap_dict:
                ...
                swap_with_model: GaussianModelActor = getattr(self, swap_with_name)
                obj_rot = self.actor_pose.get_tracking_rotation(swap_with_model.track_id, self.viewpoint_camera)
                obj_trans = self.actor_pose.get_tracking_translation(swap_with_model.track_id, self.viewpoint_camera)
            else:
                obj_rot = self.actor_pose.get_tracking_rotation(track_id, self.viewpoint_camera)
                obj_trans = self.actor_pose.get_tracking_translation(track_id, self.viewpoint_camera)
    ...
    def replace_gaussian_with_custom_actor(self, model_name, target_actor_name):
        ...
        if model_name not in self.model_paths:
            raise ValueError(f"Model name '{model_name}' not found in model paths dictionary.")
        ...
        _xyz, _features_dc, _features_rest, _opacity, _scaling, _rotation = load_ply(custom_model_path, max_sh_degree)
        ...
        _rotation = quaternion_multiply(torch.tensor(rotmat2qvec(rotation_matrix_to_x_axis.cpu().numpy()), dtype=torch.float32).unsqueeze(0).cuda(), _rotation.float())

        target_x_min, target_x_max = torch.min(actor_model._xyz[:, 0]), torch.max(actor_model._xyz[:, 0])
        target_y_min, target_y_max = torch.min(actor_model._xyz[:, 1]), torch.max(actor_model._xyz[:, 1])
        ...
        target_position = torch.tensor([target_x, target_y, target_z], dtype=torch.float32).cuda()
        ...
        actor_model._xyz = nn.Parameter(_xyz, requires_grad=True)
        actor_model._features_dc = nn.Parameter(_features_dc, requires_grad=True)
        actor_model._features_rest = nn.Parameter(_features_rest, requires_grad=True)
        actor_model._opacity = nn.Parameter(_opacity, requires_grad=True)
        actor_model._scaling = nn.Parameter(_scaling, requires_grad=True)
        actor_model._rotation = nn.Parameter(_rotation, requires_grad=True)
