from lib.utils.dair_utils import generate_dataparser_outputs
...
def readDairFullInfo(path, images='images', split_train=-1, split_test=-1, **kwargs):
    # import pdb; pdb.set_trace()
    path = path[0]
    specified_sequence_id = cfg.get('specified_sequence_id', None)
    specified_sequence_id = specified_sequence_id[0]
    selected_frames = cfg.data.get('selected_frames', None)
    if cfg.debug:
        selected_frames = [0, 0]
        
    selected_frames = selected_frames[specified_sequence_id]
    ...
    all_object_tracklets_vehicle = output['all_obj_tracklets']
    all_object_info = output['all_obj_info']
    ...
    scene_metadata['all_obj_tracklets'] = all_object_tracklets_vehicle
    scene_metadata['all_obj_info'] = all_object_info
