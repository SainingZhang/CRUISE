from lib.utils.dair_utils_single import generate_dataparser_outputs
...
def readDairFullInfo_single(path, images='images', split_train=-1, split_test=-1, **kwargs):
    ...
    output = generate_dataparser_outputs(
        ...
        cameras=cfg.data.get('cameras', [0]),
    )
    ...
    for cam in cfg.data.get('cameras', [0]):
    ...
    for i in tqdm(range(len(ixts))):
        ...
        # load ego mask
        if load_ego_mask:
            ego_mask_path = os.path.join(ego_mask_dir, f'000000_0.jpg')
