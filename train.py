...
def training():
    ...
    for iteration in range(start_iter, training_args.iterations + 1):
        ...
        if hasattr(viewpoint_cam, 'original_ego_mask'):
            ego_mask = viewpoint_cam.original_ego_mask.cuda()
        else:
            ego_mask = None
            
        if ego_mask is not None:
            mask = mask & (~ego_mask.bool())
        ...
        if hasattr(viewpoint_cam, 'original_car_mask'):
            car_mask = viewpoint_cam.original_car_mask.cuda().bool()
        else:
            car_mask = None

        # 增加一个逻辑，如果我们不渲染 obj，那么在 mask 上加上 obj_bound（即不让 obj 影响 depth mask 和 l1 mask）
        # if not gaussians.include_obj:
        #     mask = mask & (~obj_bound.bool())
        # 现在的逻辑是读取 car loss，然后添加到 mask 上
        if not gaussians.include_obj:
            mask = mask & (~car_mask.bool())
        
        if iteration % 1000 == 0:  # 每1000次迭代保存一次
            os.makedirs(f"{cfg.model_path}/mask_debug", exist_ok=True)
            
            # 创建可视化图像
            mask_vis = torch.zeros(3, mask.shape[1], mask.shape[2], device=mask.device)
            if mask is not None:
                mask_vis[0] = mask.float().squeeze(0)  # 红色通道显示普通mask
            if sky_mask is not None:
                mask_vis[1] = sky_mask.float().squeeze(0)  # 绿色通道显示sky_mask
            if ego_mask is not None:
                mask_vis[2] = ego_mask.float().squeeze(0)  # 蓝色通道显示ego_mask
                
            # 保存mask可视化结果
            save_img_torch(mask_vis, f"{cfg.model_path}/mask_debug/mask_{iteration}.jpg")
            
            # 保存obj_bound可视化结果
            if obj_bound is not None:
                obj_bound_vis = obj_bound.float().repeat(3, 1, 1)
                save_img_torch(obj_bound_vis, f"{cfg.model_path}/mask_debug/obj_bound_{iteration}.jpg")
            
            # 在特定迭代次数时插入断点
            # if iteration == training_args.debug_from:
            # if iteration == 2000:
        ...
        # # ego car loss 该方案和原本的 ego mask 方案冲突，且未测试效果，之后尝试
        # if optim_args.lambda_ego > 0 and ego_mask is not None:
        #     acc = torch.clamp(acc, min=1e-6, max=1.-1e-6)
        #     ego_loss = torch.where(ego_mask, -torch.log(1 - acc), -torch.log(acc)).mean()
        #     scalar_dict['ego_loss'] = ego_loss.item()
        #     loss += optim_args.lambda_ego * ego_loss
            
        #     # 如果有ego车的原始图像，可以直接用来替换
        #     if hasattr(viewpoint_cam, 'original_ego_image'):
        #         ego_image = viewpoint_cam.original_ego_image.cuda()
        #         image = torch.where(ego_mask.repeat(3, 1, 1), ego_image, image)
        ...
        if optim_args.lambda_reg > 0 and gaussians.include_obj and iteration >= optim_args.densify_until_iter:
            ...
            # 暂时关闭obj acc loss 不过好像不包含 obj也就不会开启这个
        ...
        with torch.no_grad():
            ...
            if iteration < optim_args.densify_until_iter:
                ...
                prune_big_points = iteration > optim_args.opacity_reset_interval  # TODO 这个步骤会报错
            ...
            training_report(tb_writer, iteration, scalar_dict, tensor_dict, training_args.test_iterations, scene, gaussians_renderer) # Forbiden due to limited spcae
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    ...
                    if hasattr(viewpoint, 'original_ego_mask'):
                        ego_mask = viewpoint.original_ego_mask.cuda()
                    else:
                        ego_mask = None    
