...
def get_obj_pose_tracking_without_remove_static(datadir, selected_frames, ego_poses, cameras=[0, 1]):
    tracklets_ls = []    
    objects_info = {}
...
def generate_dataparser_outputs(
    ...
    _, all_object_tracklets_vehicle, all_object_info = get_obj_pose_tracking_without_remove_static(
        datadir, 
        selected_frames, 
        ego_frame_poses,
        cameras,
    ...
    for track_id in all_object_info.keys():
        object_start_frame = all_object_info[track_id]['start_frame']
        object_end_frame = all_object_info[track_id]['end_frame']
        object_start_timestamp = int(vehicle_filtered_df.loc[object_start_frame,'image_timestamp']) - timestamp_offset - 0.1
        object_end_timestamp = int(vehicle_filtered_df.loc[object_end_frame,'image_timestamp']) - timestamp_offset + 0.1
        all_object_info[track_id]['start_timestamp'] = max(object_start_timestamp, min_timestamp)
        all_object_info[track_id]['end_timestamp'] = min(object_end_timestamp, max_timestamp)
    ...
    result['all_obj_tracklets'] = all_object_tracklets_vehicle
    result['all_obj_info'] = all_object_info
